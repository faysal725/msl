...
const Service = () => {
    ...
    // services.map((ser, Index) => (Index % 2  == 0) ? console.log("even", Index): console.log("odd", Index))
    // top-40 w-[250px]
                ...
                services.map((data, Index) => 
                    <div className='relative lg:pt-16' key={Index} id={`${data?.target}`}>
                        ...
                        <div className={`grid grid-cols-1 md:grid-cols-2  gap-4 max-w-7xl px-4 mx-auto mt-16 order-start`}> 
                            ...
                            {/*---------------- Images section------------------  */}
                            <div className={`${(Index % 2 !== 0) && "row-start-1 md:col-start-2" }`}>
                                <div className='relative flex justify-center sm:mr-6 mr-0 pt-10	'>
                                    <img className='w-[82%] h-[400px]' src="/Assets/Service/Rectangle.svg" alt="" />
                                    <img className={`absolute ${(Index % 2 === 0)? "top-40 md:top-32 w-[250px] md:w-[380px] md:right-0 md:bottom-0" : "top-32 lg:right-40 md:bottom-0"}`} src={data.imageUrl} alt="" />
                                </div>
                            </div>
                            ...
                            <div className={`${(Index % 2 === 0)? "order-last" : "order-first"}`}>
                                ...
                                <div className='flex mt-10'>
                                    <Link to="/portfolio" >
                                        <button
                                        type="button"
                                        className="transition inline-flex items-center px-6  py-3 border border-transparent text-xl	 font-medium rounded-full shadow-sm text-white bg-[#00b4ff] hover:bg-[#347deb] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 "
                                        >
                                        View Example
                                        </button>
                                    </Link>
